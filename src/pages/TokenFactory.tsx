...
export function TokenFactory() {
  ...
  const startDeploy = () => {
    if (!form.name || !form.symbol) { toast.error("الرجاء تعبئة الاسم والرمز"); return; }
    setStep("review");
    setOpen(true);
  };
              ...
              {features.map((f) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-right p-5 border rounded-sm flex items-start gap-4 transition-all ${on ? "border-foreground bg-foreground/[0.03]" : "border-border hover:border-foreground/40"}`}
                  >
                    <div className={`size-5 shrink-0 rounded-sm border-2 grid place-items-center mt-0.5 transition-colors ${on ? "bg-foreground border-foreground" : "border-border"}`}>
        ...
        <aside>
          <div className="lg:sticky lg:top-28 space-y-6">
            <div className="border border-border rounded-sm p-6 bg-card">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-4">ملخص العقد</p>
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              سيتم تشفير المفتاح الخاص للمحفظة باستخدام Fernet ولن يُكشف بعد التوقيع.
            </p>
          </div>
        </aside>
      </div>

      <Modal open={open} onClose={() => { if (step !== "deploying") { setOpen(false); setTimeout(() => setStep("review"), 300); } }}>
        {step === "review" && (
          <>
            <h2 className="text-2xl font-bold tracking-tight mb-5">تأكيد نشر العقد</h2>
            <div className="space-y-4">
              ...
              <div className="p-4 border border-border bg-muted/30 rounded-sm text-xs leading-relaxed">
                ⚠ بمجرد النشر لا يمكن تعديل الكود الأساسي للعقد. تأكد من جميع البيانات.
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setOpen(false)} className="px-5 py-3 border border-border rounded-sm font-semibold hover:bg-muted">
                  إلغاء
                </button>
                <button onClick={confirmDeploy} className="flex-1 py-3 bg-accent text-accent-foreground font-bold rounded-sm hover:brightness-110 inline-flex items-center justify-center gap-2">
                  <Rocket className="size-4" strokeWidth={2.5} /> تأكيد ونشر العقد الآن
                </button>
              </div>
            </div>
          </>
        ...
        {step === "deploying" && (
          <div className="text-center py-8">
            ...
            <p className="text-xl font-bold mb-2">جارٍ نشر العقد...</p>
            <p className="text-sm text-muted-foreground">ترجمة → تقدير الغاز → التوقيع → النشر على {form.network}</p>
            <div className="mt-6 max-w-xs mx-auto h-1 bg-border rounded-full overflow-hidden">
        ...
        {step === "success" && (
          <>
            <div className="text-center mb-6">
              ...
              <p className="text-2xl font-bold mb-2">تم النشر بنجاح</p>
              <p className="text-sm text-muted-foreground">عقد {form.symbol || "TKN"} نشط الآن على {form.network}.</p>
            </div>
            <div className="p-4 bg-muted/50 rounded-sm mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => { navigator.clipboard.writeText(txHash); toast.success("تم النسخ"); }} className="text-muted-foreground hover:text-foreground">
                  <Copy className="size-3.5" />
                </button>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={reset} className="px-5 py-3 border border-border rounded-sm font-semibold hover:bg-muted">
                نشر آخر
              </button>
              <Link to="/assets" onClick={reset} className="flex-1 py-3 bg-foreground text-background font-bold rounded-sm hover:bg-foreground/90 inline-flex items-center justify-center gap-2">
                عرض في سجل الأصول <ExternalLink className="size-4" />
              </Link>
            </div>
          </>
        )}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section>
      <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-6 pb-3 border-b border-border">{title}</h2>
      {children}
    </section>
  );
}
function Field({ label, placeholder, mono, value, onChange }: { label: string; placeholder?: string; mono?: boolean; value?: string; onChange?: (v: string) => void }) {
  return (
    <label className="block">
      <span className="text-sm font-semibold mb-2 block">{label}</span>
      <input placeholder={placeholder} value={value} onChange={(e) => onChange?.(e.target.value)} dir={mono ? "ltr" : "rtl"}
