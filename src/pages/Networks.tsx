...
import {
  Plus, Trash2, CheckCircle2, XCircle, Loader2, Wifi, WifiOff, Edit2, X, Check,
} from "lucide-react";
import {
...
const empty: Omit<Network, "id"> = { name: "", chainId: "", rpc: "", currency: "", explorer: "", status: "inactive" };

export function Networks() {
  const [nets, setNets] = useState<Network[]>(initial);
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<Network | null>(null);
  const [form, setForm] = useState<Omit<Network, "id">>(empty);
  const [testId, setTestId] = useState<number | null>(null);
  const [saving, setSaving] = useState(false);

  const openAdd = () => { setEditing(null); setForm(empty); setOpen(true); };
  const openEdit = (n: Network) => { setEditing(n); setForm({ name: n.name, chainId: n.chainId, rpc: n.rpc, currency: n.currency, explorer: n.explorer, status: n.status }); setOpen(true); };

  const save = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name || !form.rpc || !form.chainId || !form.currency) {
  ...
  const remove = (n: Network) => {
    setNets((all) => all.filter((x) => x.id !== n.id));
    toast.success(`تم حذف ${n.name}`);
  };
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10 animate-enter">
        {[
          { l: "إجمالي الشبكات", v: String(nets.length) },
          { l: "شبكات نشطة", v: String(active) },
          { l: "متوسط زمن الاستجابة", v: `${Math.round(avgLatency)} ms` },
        ].map((s) => (
      ...
      <div className="space-y-3 animate-enter [animation-delay:150ms]">
        {nets.map((n) => (
          <div
            key={n.id}
            className="border border-border rounded-sm p-5 sm:p-6 flex flex-wrap gap-4 items-center justify-between hover:border-foreground/30 transition-colors"
          >
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button
                onClick={() => testConn(n)}
                disabled={testId === n.id || n.status === "testing"}
              ...
              <button
                onClick={() => openEdit(n)}
                className="px-3 py-1.5 text-xs font-semibold border border-border rounded-sm hover:bg-muted transition-colors inline-flex items-center gap-1.5"
              >
                <Edit2 className="size-3" /> تعديل
              </button>
              <button
                onClick={() => toggle(n)}
                className={`px-3 py-1.5 text-xs font-semibold rounded-sm transition-colors inline-flex items-center gap-1.5 ${
                  n.status === "active"
              ...
              <button
                onClick={() => remove(n)}
                className="size-8 grid place-items-center text-muted-foreground hover:text-destructive transition-colors"
              >
        ...
        <DialogContent className="max-w-lg" dir="rtl">
          <DialogHeader>
            <DialogTitle className="text-2xl tracking-tight flex items-center gap-3">
              ...
              {editing ? "تعديل الشبكة" : "إضافة شبكة جديدة"}
            </DialogTitle>
            <DialogDescription>
              {editing ? "عدّل إعدادات الشبكة وبيانات RPC." : "أضف شبكة EVM جديدة للاتصال بها."}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={save} className="space-y-5 mt-2">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <label className="block">
                ...
                <input
                  required value={form.chainId} onChange={(e) => setForm({ ...form, chainId: e.target.value })}
            ...
            <label className="block">
              ...
              <input
                required value={form.rpc} onChange={(e) => setForm({ ...form, rpc: e.target.value })}
                placeholder="https://mainnet.infura.io/v3/..." dir="ltr"
                className="w-full bg-transparent border border-border rounded-sm px-4 py-3 outline-none focus:border-foreground font-mono text-sm"
              />
            </label>
            <div className="grid sm:grid-cols-2 gap-4">
              <label className="block">
                ...
                <input
                  required value={form.currency} onChange={(e) => setForm({ ...form, currency: e.target.value })}
                  placeholder="ETH" dir="ltr"
                  className="w-full bg-transparent border border-border rounded-sm px-4 py-3 outline-none focus:border-foreground font-mono text-sm"
                />
              </label>
              <label className="block">
                ...
                <input
                  value={form.explorer} onChange={(e) => setForm({ ...form, explorer: e.target.value })}
                  placeholder="https://etherscan.io" dir="ltr"
                  className="w-full bg-transparent border border-border rounded-sm px-4 py-3 outline-none focus:border-foreground font-mono text-sm"
                />
              </label>
            </div>
